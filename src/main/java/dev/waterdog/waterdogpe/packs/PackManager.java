...
public class PackManager {
    ...
    public void rebuildPackets() {
        ...
        for (ResourcePack pack : this.packs.values()) {
            ...
            ResourcePacksInfoPacket.Entry infoEntry = this.createInfoEntry(pack, pack.getCdnUrl(), isBehaviorPack);
        ...
        infoPacket.setHasAddonPacks(hasAddonPacks);
    ...
    /**
     * Build protocol-version-aware ResourcePacksInfoPacket, also honoring CDN platform filtering.
     * v729+: behaviorPackInfos is no longer serialized, merge behavior packs into resourcePackInfos.
     */
    public ResourcePacksInfoPacket buildPacksInfoPacket(ProtocolVersion protocol, Platform platform) {
        ResourcePacksInfoPacket source = this.getPacksInfoPacket(platform);
        ResourcePacksInfoPacket packet = new ResourcePacksInfoPacket();
        packet.setForcedToAccept(source.isForcedToAccept());
        packet.setWorldTemplateId(source.getWorldTemplateId());
        packet.setWorldTemplateVersion(source.getWorldTemplateVersion());
        packet.setHasAddonPacks(source.isHasAddonPacks());
        packet.setScriptingEnabled(source.isScriptingEnabled());
        packet.setForcingServerPacksEnabled(source.isForcingServerPacksEnabled());

        if (protocol.isAfterOrEqual(ProtocolVersion.MINECRAFT_PE_1_21_30)) {
            // v729+: merge behavior packs into resource pack list
            packet.getResourcePackInfos().addAll(source.getResourcePackInfos());
            packet.getResourcePackInfos().addAll(source.getBehaviorPackInfos());
        } else {
            packet.getResourcePackInfos().addAll(source.getResourcePackInfos());
            packet.getBehaviorPackInfos().addAll(source.getBehaviorPackInfos());
        }
        return packet;
    }
